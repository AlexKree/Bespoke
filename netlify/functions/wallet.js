...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (action === 'add-funds') {
      ...
      try {
        await db.query(
          `INSERT INTO wallet_transactions (user_id, type, amount_cents, label) VALUES ($1, 'deposit', $2, 'Dépôt de fonds')`,
          [user.id, amount]
        );
    ...
    if (action === 'allocate') {
      ...
      try {
        await db.query(
          `INSERT INTO wallet_transactions (user_id, type, amount_cents, label) VALUES ($1, 'allocation', $2, $3)`,
